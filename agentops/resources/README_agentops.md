...
The Databricks CLI bundles top level is defined by file `agentops/databricks.yml`.
...
Agent Resource Configurations in this directory:
 - data preparation workflow (`agentops/resources/data-preparation-resource.yml`)
 - agent development and deployment workflow (`agentops/resources/agent-resource.yml`)
 - app deployment workflow (`agentops/resources/app-deployment-resource.yml`)
 - model definition, experiment, and app definition (`agentops/resources/agents-artifacts-resource.yml`)
...
When you initialize the stack, we set the catalog name in the `agentops/databricks.yml`, so we expect a catalog of the same name in each environment. I
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|
| dev         | The `dev` deployment target is used to deploy resources to development workspace with `dev` configs. The config is for project development purposes.                                                           | dev workspace        | dev-agentops-model     | /dev-agentops-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-agentops-model | /staging-agentops-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-agentops-model    | /prod-agentops-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and agent resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-agentops-model    | /test-agentops-experiment    |
...
2. Complete the `TODO` in `agentops/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
To get started, open `agentops/resources/batch-inference-workflow-resource.yml`.  The file contains the agent resource definition of a data ingestion job, like:

```$xslt
resources:
  jobs:
    data_preprocessing_job:
      name: ${bundle.target}-agentops-data-preprocessing-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-agentops-data-preprocessing-job`
that runs the notebook under `agentops/data_preparation/data_ingestion/notebooks/DataIngestion.py` to ingest documents from our website. 
...
We specify a `data_preprocessing_job` under `resources/jobs` to define a Databricks workflow with internal key `data_preprocessing_job` and job name `{bundle.target}-agentops-data-preprocessing-job`.
The workflow contains a single task with task key `data_preprocessing_job`. The task runs notebook `agentops/data_preparation/data_ingestion/notebooks/DataIngestion.py` with provided parameters `uc_catalog`, `schema`, `raw_data_table`, and `data_source_url` passing to the notebook.
...
`test-agentops--data-preprocessing-job`. During the deployment of the `staging` environment config, the job name will be
`staging-agentops--data-preprocessing-job`.
